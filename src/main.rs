...
#[cfg(not(target_arch = "wasm32"))]
use web_max_area_data_collection::vertex::{TriangulationType, generate_random_triangles_in_buckets};
use std::fs::write;
use web_max_area_data_collection::{run, vertex::{Vertex, generate_circle_type_one, generate_circle_type_three, generate_circle_type_two}};
...
fn create_triangulation_file() {
    ...
    // let data = generate_random_triangles_in_buckets(200.0, 500_000.0, 10, 10, 0.75, 262_144).into_iter().map(|(v, i)| (TriangulationType::Random, v, i)).collect::<Vec<_>>();
    // let data = postcard::to_allocvec(&data).unwrap();
